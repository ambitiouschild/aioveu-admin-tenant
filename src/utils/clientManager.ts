...
export const CLIENT_CONFIG = {
  ...
  getBASIC_AUTH(): any {
    ...
    try {
      ...
      // 示例3: 验证认证
      const result = validateClientAuth(basicAuth, this.CLIENT_ID, this.CLIENT_SECRET);
...
export const parseBasicAuth = (auth: any) => {
  ...
  const decoded = typeof atob !== `undefined` ? atob(base64) : base64Decode(base64);
...
// 示例3: 验证认证
const result = validateClientAuth("Basic bWFsbC1hcHA6MTIzNDU2", "mall-app", "123456");
console.log(`验证结果:`, result); // { valid: true, clientId: 'mall-app' }
